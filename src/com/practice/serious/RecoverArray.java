...
public class RecoverArray {
  public static void main(String[] args) {
    RecoverArray me = new RecoverArray();
  }

  public int[] recoverArray(int n, int[] sums) {
    int total = 0;
    for (int num : sums) total += num;
    int sum = total / (1 << n - 1);
    Arrays.sort(sums);
    int[] arr = new int[n];
    List<Integer> sumsList = new ArrayList<>();
    for (int num : sums) sumsList.add(num);
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (!selected[j]) {
          ...
          for (int k = j + 1; k < size; k++) {
            if (!selected[k] && sumsList.get(k) == sumsList.get(j) + minNum) {
              ...
              if (minNum == sumsList.get(k)) flag = true;
              break;
            }
          }
        }
      ...
      sumsList.clear();
      if (flag) sumsList = new ArrayList<Integer>(small);
      else {
